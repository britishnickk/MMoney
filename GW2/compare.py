...
#Load names corresponding to the item id's
item_details={}
with open('recipe_created_items.json') as data_file:
	item_details = json.load(data_file)
...
for i in range(top):
	print "For "+str(data[i][0])+" craft "+ str(data[i][1][1])+" "+item_details[str(data[i][1][0])]['name']+" using:"
	try:
		for j in range(2,len(data[i][1])):
			if j%2==1:
				continue
			print ""+str(data[i][1][j+1])+" "+item_details[str(data[i][1][j])]['name']+" ("+items[data[i][1][j]]+")"	
	except KeyboardInterrupt:
		exit()
	except:
		print "\tWe're sorry, the following item id does not have a name in our database:"	
		print ""+str(data[i][1][j+1])+" "+str(data[i][1][j])+" ("+items[data[i][1][j]]+")"	
